        ...
        try:
            ...
            if function_calls:
                ...
                if any(s in output['content'] for s in [cls.FN_ARGS, cls.FN_EXIT, cls.FN_NAME, cls.FN_RESULT]):
                    ...
                    raise e
                response = re.sub(f"<{thinking_tag}>.*?</{thinking_tag}>", "", output['content'],
                                  flags=re.DOTALL).strip()
                response = response.replace(f'<{cls.fix_reply_tag}>', "").replace(f'</{cls.fix_reply_tag}>',
                                                                                  "").replace(f"</{thinking_tag}>",
                                                                                              "").strip()
